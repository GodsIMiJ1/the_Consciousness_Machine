...
const LatticeViewer = () => {
  const { lattice, loading, error, setActiveView } = useLatticeState();
  const { selectedNode, hoveredNode, selectNode, hoverNode } = useLatticeSelection();

  const selectedView = lattice.active_view;

  // Get data from lattice state
  const shardData = lattice.shards;
  const crownData = lattice.crowns[0]; // Trinity Flame Genesis Crown
  const grandCrownProgress = {
    current: lattice.statistics.trinity_progress.current_crowns,
    required: lattice.statistics.trinity_progress.required_for_grand,
    percentage: lattice.statistics.trinity_progress.percentage,
    ...
    return (
      <div className="min-h-screen bg-slate-900 text-white p-6 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-300">Loading Memory Lattice...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-900 text-white p-6 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-red-400">Error loading lattice: {error}</p>
        </div>
      </div>
    );
  }
            ...
            <Card className="bg-slate-800 border-slate-700">
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Tabs value={selectedView} onValueChange={setActiveView} className="w-auto">
          ...
          <div>
            <Card className="bg-slate-800 border-slate-700">
              ...
              <CardContent className="space-y-4">
                {hoveredNode ? (() => {
                  const nodeData = [...lattice.shards, ...lattice.crowns, ...lattice.grand_crowns]
                    .find(item => item.id === hoveredNode);
                    ...
                    <div className="space-y-3">
                      <div>
                        <h4 className="font-semibold text-yellow-400 mb-1">
                          {nodeData.title || 'Trinity Flame Genesis'}
                        </h4>
                        <div className="text-sm text-slate-300 space-y-1">
                          <div>ID: <code className="text-xs bg-slate-900 px-1 rounded">{nodeData.id?.substring(0, 8)}...</code></div>
                          <div>Coordinates: <span className="text-orange-400">
                            {'lattice_coordinates' in nodeData ? nodeData.lattice_coordinates :
                             'coordinates' in nodeData ? nodeData.coordinates : 'N/A'}
                          </span></div>
                          {'agent' in nodeData && <div>Agent: <span className="text-blue-400">{nodeData.agent}</span></div>}
                          {'flame_sealed' in nodeData && nodeData.flame_sealed && <div>Status: <span className="text-green-400">🔐 Flame Sealed</span></div>}
                          {'sealed' in nodeData && nodeData.sealed && <div>Status: <span className="text-green-400">🔐 Flame Sealed</span></div>}
                        </div>
                      </div>

                      {'seal_hash' in nodeData && nodeData.seal_hash && (
                ...
                <div className="border-t border-slate-600 pt-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="text-center p-2 bg-slate-900/30 rounded">
                      <div className="text-lg font-bold text-orange-400">{lattice.statistics.total_shards}</div>
                      <div className="text-xs text-slate-400">Shards</div>
                    </div>
                    <div className="text-center p-2 bg-slate-900/30 rounded">
                      <div className="text-lg font-bold text-yellow-400">{lattice.statistics.total_crowns}</div>
                      <div className="text-xs text-slate-400">Crowns</div>
                    </div>
                    <div className="text-center p-2 bg-slate-900/30 rounded">
                      <div className="text-lg font-bold text-purple-400">{lattice.statistics.grand_crowns}</div>
                      <div className="text-xs text-slate-400">Grand Crowns</div>
                    </div>
                    <div className="text-center p-2 bg-slate-900/30 rounded">
                      <div className="text-lg font-bold text-green-400">{lattice.statistics.sealed_crowns}</div>
