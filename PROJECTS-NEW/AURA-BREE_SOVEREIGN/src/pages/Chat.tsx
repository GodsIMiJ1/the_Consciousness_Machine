...
import { Send, Volume2, Wifi, WifiOff, Flame } from "lucide-react";
import { getDeviceId } from "@/lib/device";
import { loadMessages, saveMessages } from "@/lib/chatStorage";
import type { ChatMessage } from "@/lib/chatStorage";
import { chat, isFlameRouterConfigured, getProviderStatus } from "@/lib/flameRouter";
import { getChatResponse, isOpenAIConfigured } from "@/lib/openai"; // Legacy fallback
...
useEffect(() => {
  ...
  const sendMessage = async (content: string) => {
    ...
    // Check if FlameRouter or legacy OpenAI is configured
    if (!isFlameRouterConfigured() && !isOpenAIConfigured()) {
      const errorMessage: ChatMessage = {
        id: (Date.now() + 2).toString(),
        role: 'assistant',
        content: 'No AI providers are configured. Please set up Ollama, LM Studio, Hugging Face, or OpenAI in your .env file.',
        timestamp: Date.now()
      };
...
const userMessage: ChatMessage = {
  ...
  timestamp: Date.now()
    ...
    try {
      ...
      // Try FlameRouter first (multi-provider with failover)
      if (isFlameRouterConfigured()) {
...
- Avoid diagnosis or medical claims. Provide crisis resources if user expresses intent to harm.`;
        ...
        text = await getChatResponse(chatMessages);
    ...
    <div className="flex flex-col h-full bg-background">
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-card/50 border-b border-border">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-medium">FlameRouter</span>
          {currentProvider && (
        ...
        <div className="flex items-center gap-1">
          ...
          {isFlameRouterConfigured() ? (
            <Wifi className="w-4 h-4 text-green-500 ml-2" />
          ) : (
